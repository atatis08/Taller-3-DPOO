...
public class Aerolinea
{
    ...
    {
        return (Collection<Cliente>) clientes.values( );
    }
    ...
    public Collection<Tiquete> getTiquetes( )
    {
        ...
        return (Collection<Tiquete>) tiquetes;
    ...
    public void programarVuelo( String fecha, String codigoRuta, String nombreAvion ) throws Exception
    {
        ...
        for (Avion avionS : aviones) {
            if (avionS.getNombre().equals(nombreAvion)) {
                avion = avionS;
                break;
            }
        }
        if (avion == null) {
        	 throw new Exception("El avion no se encontro o no existe");

        }
        Ruta ruta = getRuta(codigoRuta);
        if (ruta == null) {
            throw new Exception("Esa ruta no esta disponible");
        }
     ...
     */
    public void venderTiquetes( String identificadorCliente, String fecha, String codigoRuta, int cantidad ) throws VueloSobrevendidoException, Exception{
    	Cliente cliente = getCliente(identificadorCliente);
    if (cliente == null) {
        throw new Exception("El identificador " + identificadorCliente + " no existe.");
     }
    
    Ruta rutaVuelo = rutas.get(codigoRuta);
    
    Vuelo flight = getVuelo(codigoRuta, fecha);
    
    if (flight == null) {
    	throw new Exception("El vuelo no existe");
    }
    
    if (flight.getTiquetes().size() + cantidad > ((Avion) flight.getAvion()).getCapacidad()) {
        throw new VueloSobrevendidoException(flight);
    }
    ...
    public CalculadoraTarifas definirCalculadoraTemporada(String fecha) {
		...
		if ((nMes >= 1 && nMes <= 5) || (nMes >= 9 && nMes <= 11)) {
            return new CalculadoraTemporadaBaja();
        } else {
    ...
    public String consultarSaldoPendienteCliente( String identificadorCliente )
    {
    	Cliente cliente = getCliente(identificadorCliente);
        if (cliente == null) {
            return "El identificado " + identificadorCliente + " no existe o no se encontro.";
        }
        
        int valor = cliente.calcularValorTotalTiquetes();

        return "El saldo pendiente  es:"  + valor;
